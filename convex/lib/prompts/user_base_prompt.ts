...
{{#if durationSeconds}}- Target video length: {{durationSeconds}} seconds (modulate beat count to fit). {{/if}}
{{#if maxPromptChars}}
- Provider video prompt character limit: {{maxPromptChars}} (videoScenes will be flattened into one text prompt;
stay concise and pricise but details that would enrich video scene should not be cut down either).
{{/if}}
